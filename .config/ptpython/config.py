...
"""
from prompt_toolkit.filters import ViInsertMode, ViMode,ViSelectionMode,ViNavigationMode
...
def configure(repl):
    ...
    # Up down in normal, C-U still dels line
    @repl.add_key_binding("c-u",filter=ViNavigationMode())
    def _(event):
        "Make up feel like up"
        event.cli.current_buffer.history_backward()


    @repl.add_key_binding("c-d",filter=ViNavigationMode())
    def _(event):
        "Make down feel like down "
        event.cli.current_buffer.history_forward()

    @repl.add_key_binding("c-t")
    def _(event):
        " Pressing Control-t import monkeyhex"
        event.cli.current_buffer.insert_text("import monkeyhex;")
